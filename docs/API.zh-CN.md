...
若包内含 `meta.json`（由 §5.7 导出的备份自带），其中的**品牌**（app_name / app_subtitle / html_title）会在导入配置后一并恢复；`sort` / `log_view_since` 等不会被改动，避免导入意外重排或隐藏日志。

返回（`branding_restored` 表示是否从备份还原了品牌）：

```json
{ "imported": ["diag1", "diag2"], "branding_restored": true }
```
