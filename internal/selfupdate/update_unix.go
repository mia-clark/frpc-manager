...
func spawnUpdater(u *Updater, mode Mode, targetVersion string) error {
	...
	if mode == ModeSystemd && hasExec("systemd-run") {
		...
		// aborts with "Unit frpcmgrd-selfupdate.service already exists" (--collect
		...
		_ = exec.CommandContext(stopCtx, "systemctl", "stop", "frpcmgrd-selfupdate.service").Run()
		cancel()
		_ = exec.Command("systemctl", "reset-failed", "frpcmgrd-selfupdate.service").Run()

		out, err := run("frpcmgrd-selfupdate")
		if err != nil && strings.Contains(string(out), "already exists") {
			...
			out, err = run(fmt.Sprintf("frpcmgrd-selfupdate-%d", time.Now().UnixNano()))
		}
		if err != nil {
